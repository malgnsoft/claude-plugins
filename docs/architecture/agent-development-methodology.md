...
4. **역량이 여러 에이전트에 공통으로 필요한가?** — 2개 이상의 기존 에이전트가 동일 절차를 필요로 하면 새 에이전트가 아니라 **스킬**이다(에이전트를 늘리면 같은 절차가 N군데 중복 인라인된다). 예: `domain-backend-api-security`는 backend-dev 전용이 아니라 architect·security와도 공유되는 절차라 스킬로 분리되어 있다.
...
### 3.2 링크 방향 (규칙 2, 절대)
Skill → Knowledge 참조는 허용(예: backend-dev.md의 "ℹ️ Skill: domain-backend-api-security 참조"), **Knowledge → Skill 참조는 금지**. Knowledge가 Skill을 "증명"하려 들면 순환 참조가 생긴다. 위반 예시를 찾으면(예: 배경 설명 문서가 "따라서 X 스킬을 따르라"고 지시형으로 끝맺으면) 그 지시문은 Skill로 옮기고 Knowledge에는 배경만 남긴다.
...
### 5.1 중복 판정 — "80% 겹침 테스트"
신규 산출물(에이전트/스킬/knowledge) 작성 전, 기존 자산이 요구의 80% 이상을 이미 커버하는지 먼저 확인한다(`grep -r` 키워드 검색 1회). 80% 이상 겹치면 **새로 만들지 않고 기존 자산을 확장**한다. 이 저장소에는 실패 사례와 그 교정 사례가 모두 있다. 보안 스킬 4종은 각 `description`에 "이 스킬은 그와 중복 없이…"라는 상호 해명문을 달아 경계를 표시했지만, **해명문이 필요하다는 사실 자체가 선택 지점이 모호하다는 증거**였다. 2026-08-24 정리에서 실제 교집합을 실측한 결과 두 스킬(원론 ↔ 스택 구현형)은 서로를 "먼저 참조하십시오"라고 가리키고 있었다 — **항상 함께 읽히도록 설계된 두 스킬은 하나여야 한다**는 신호라 한쪽을 흡수해 3종으로 줄였다. 반면 나머지 쌍은 실측상 교집합이 거의 없었고, 문제는 내용이 아니라 이름과 트리거 문구였다.

따라서 감사 원칙은 다음과 같다:
- 상호 해명문("~와 중복되지 않는다")을 경계 표시 수단으로 인정하지 않는다. 대신 각 description이 **"언제 이걸 여는가"를 긍정형으로, 서로 배타적으로** 진술하는지 본다(예: 라우트 한 건 / 이 스택의 코드베이스 / 프로젝트 전체 태세 — 하나의 축으로 갈린다).
- 겹침이 의심되면 실제 체크리스트 항목을 나열해 교집합을 **실측**한다. 겹치면 통합 후보, 안 겹치는데 헷갈리면 이름·트리거 재작성 대상이다.
- `description`은 모든 세션에 상시 로드되므로 해명문은 쓰이든 안 쓰이든 매번 비용을 문다. 반면 SKILL.md 본문은 invoke 시에만 로드되는 조건부 비용이다 — **함께 읽히는 두 스킬을 하나로 합치면 상시 비용은 줄고 본문 비용은 늘지 않는다**(어차피 둘 다 열렸을 것이므로).
