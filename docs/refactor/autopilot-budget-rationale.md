...
이어 reviewer 풀패널 반려 수리에서 16.4→17.7 KB(+1,394 B)가 됐다. 늘어난 자리는 셋이다: ①VERIFY 단계가 **PM이 사람 승인 없이 닫은 건의 근거(reviewer 보고서 경로·판정, 필요 시 evaluator 기록)를 실재로 대조**하게 하는 문장 — autopilot이 이미 지키는 claimed≠verified를 승인 근거에도 적용하는 것이고, 이 문장이 없으면 "검증했다"는 말만으로 사람 승인이 대체된다 ②그 확인을 자기검증 체크리스트에 짝으로 건 항목 ③포괄 위임 전제(사용자가 이 목표에서 허용한 자율 범위)를 착수 조건으로 복원한 문장 — 건별 동의를 없앤 모델이라 앞단의 포괄 동의가 유일한 동의 획득 지점이다.
