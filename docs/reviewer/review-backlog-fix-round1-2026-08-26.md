...
# 2차 — 지적 반영 재검증 (targeted, 축소 모드)

리뷰 대상: `git diff c58d8e8..10fed1c` (11파일, 그중 제품·스크립트 4파일 +8/−8)
target_id: `backlog-fix-round1` · 리스크 범주: 전역 자동실행 자산 · 작업 등급: Sensitive
모드 판정: PM 위임에 **재검토 3요소(target_id · 직전 리뷰 경로 · 리스크 범주)가 전부 명시**돼 있고, 델타가 1차 지적의 반영분에 한정되며 새 리스크 표면을 열지 않았다 → **축소(C) 모드**. 풀패널 강제 승격 조건(신규 실행경로·권한경계 변경 등) 해당 없음을 diff로 확인했다.

**이번 델타 판정: 🟢 Green** — Critical 0 · Major 0 · Minor 1(신규) · Nit 3(신규)
**트랙 누계**: 1차 지적 8건 중 **해소 4 · 기각검토 후 강등 1 · 보류 타당 3**. **병합 차단 사유 없음.**

## 페르소나 재사용 판정 (2차)

`docs/reviewer/personas/INDEX.md`를 Read해 역할개념 열을 대조했다. **신규 0건 — 4개 전부 재사용**, 그중 델타 관련 4명 전원 투입(델타가 4개 관점을 모두 건드린다).

| 페르소나 | 재사용/신규 | 이번 델타에서 맡은 것 |
|---|---|---|
| persona-spec-implementation-conformance-auditor.md | **재사용** | RV-201 등록치 ↔ 실측 대조, 사유서 논거 ↔ 실제 증분 대조 |
| persona-script-skill-consistency-auditor.md | **재사용** | RV-204·205 문면 ↔ hub 응답 실물/스크립트 필드 대조 |
| persona-field-executability-officer.md | **재사용** | RV-203 관례 실측, RV-206 경로③ 재현 실행, trainer 신규 grep 실행 |
| persona-process-mechanism-zero-based-challenger.md (**발산**) | **재사용** | RV-202/207/208 보류 판단의 타당성, 1,936 B 단일 체크박스의 비례성 |

**제약 준수 고지**: 이번 라운드는 "리뷰 보고서 갱신만 허용" 제약이 있어 **페르소나 파일의 '적용 이력' append와 INDEX.md '최근 재사용' 열 갱신을 하지 않았다.** 평소 절차상 해야 하는 일이므로, 제약이 풀리면 PM이 그 append를 지시하거나 다음 라운드 reviewer가 이 절을 근거로 소급 기록해야 한다.

## 1차 지적별 처리 판정

| # | 1차 심각도 | 판정 | 근거 (확인방법) |
|---|---|---|---|
| RV-201 | 🟠 Major | **✅ 해소** | `wc -c malgn-agent/agents/evaluator.md` = **24,131 B**, `scripts/validate-agent-assets.mjs:83` 등록치 = **24131** → 차이 0 B. 브랜치 실행: `ERROR 0 · WARN 18 · INFO 2`, evaluator.md가 `INFO [BUDGET_RATIONALE_OK]`로 복귀. **베이스 복귀 여부는 이번엔 계산이 아니라 실행으로 확인** — 베이스 `43f2040` 트리를 스크래치패드에 풀어 그 시점 스크립트를 그대로 돌린 결과도 `ERROR 0 · WARN 18 · INFO 2`(1차 보고서의 "계산으로 대체" 미확인 항목을 실측으로 승격). 사유서 논거도 실제 증분과 맞는다: 사유서가 새로 변호하는 양이 +약 2,000 B(접두어 게이트) + 약 800 B(점수 왕복) = 약 2,800 B이고, 실측 증분은 24,131 − 21,353 = **2,778 B**로 일치 |
| RV-203 | 🟡 Minor | **⚪ Nit 강등 + 백로그** (PM 기각을 **절반만** 수용) | 아래 "RV-203 기각 독립 판정" 참조 |
| RV-204 | 🟡 Minor | **✅ 해소 (라이브 실측으로 확정)** | `agent_get_context(agentName:"trainer", scoreHistoryLimit:3)` **실호출** → 응답 최상위 = `{agentName, latestScore, scoreHistory, recentLearnings}`, `latestScore` = `{overallScore: 69.9, dimensionScores, improvementNote, createdAt}`. 새 문면의 `latestScore.overallScore`와 최상위 키 4개 열거가 **응답 실물과 정확히 일치**한다(1차에서 "미확인"으로 남겼던 내부 필드명을 이번에 실측했다 — 이 계정에 점수 이력이 있는 에이전트를 찾아 조회). `latestScore === null` → "최초 회차" 판별도 스키마와 정합 |
| RV-204 부수 | — | **정본과 모순 없음** | `skills/domain-training-scorecard-eval/SKILL.md:97`("최신 **기록의** `overallScore`")과 나란히 대조 — `latestScore.overallScore`가 곧 "최신 기록의 overallScore"라 두 서술은 같은 값을 가리킨다. 이력 없음 처리도 정합(SKILL.md "필드를 생략한다" ↔ 가이드 "최초 회차로 적는다"는 각각 스크립트 입력/보고서 표기라 층위가 다르다). 잔여는 **정밀도 비대칭**뿐 — 정본(SKILL.md)이 파생 문서(knowledge)보다 덜 구체적이다. 지적으로 올리지 않는다(오독을 낳지 않으며, 변경 동결 하에서 근거가 약하다) |
| RV-205 | 🟡 Minor | **✅ 해소, 새 오독 없음** | `:124`·`:138` 두 자리 모두 "Scorecard 입력 JSON의 `previousScore` 필드(`bin/calc-training-scorecard.mjs`)"로 귀속되고, `:138` 말미에 "`agent_score_record`의 파라미터가 아닙니다"가 명시됐다. 실물 대조: `bin/calc-training-scorecard.mjs:59`(입력 JSON 주석, "선택"), `:304-316 judgeTrend`, `skills/domain-training-scorecard-eval/SKILL.md:76·93·97` 전부 동일 귀속. 새 문장이 만드는 다른 오독은 찾지 못했다 — "이 스크립트의 입력 필드"라는 지시대명사가 바로 앞 괄호의 스크립트를 가리켜 모호하지 않다 |
| RV-206(a) 자기부정 | 🟡 Minor | **✅ 해소** | 옛 문면 "판정축은 스킬명 grep 히트 수가 **아니라**"(→ 뒤에서 숫자 구간을 쓰는 자기부정)가 삭제되고 "비용 구조는 도달 범위로 재되, **세는 범위가** 스킬명 직접 히트가 아니라 세 경로의 합산"으로 대체됐다. 부정 대상이 판정축→세는 범위로 옮겨져, 이어지는 "5개 이상/2~4개/1개" 구간과 모순되지 않는다 |
| RV-206(b) 경로③ 재현 | 🟡 Minor | **✅ 해소 (내가 규칙대로 직접 재현)** | 새 규칙("고정 문자열은 그 SKILL.md가 정본으로 지목하는 파일 경로·도구명·커맨드로 한정, 자연어 배제")대로 검색어를 뽑았다: `skills/common-product-principles-reference/SKILL.md:10`이 `docs/product-principles.md`를 정본으로 지목 → 검색어 `product-principles` → `grep -rl product-principles agents/*.md` = **13개**. 1차에서 내가 즉흥으로 잡았던 검색어와 같은 결론(PASS)에 도달하며, 이번엔 **검색어 도출 경로가 문면에 적혀 있어 다음 판정자도 같은 수를 낸다** |
| RV-206(b) 과잉협소 여부 | — | **정당한 스킬 반려 0건 확인** | `common-*` 9종 전수 점검: 경로③에 의존하는 것은 `common-product-principles-reference` 1종뿐이고(②knowledge 경유 = `grep -rl common-product-principles-reference knowledge/` → **0건**), 나머지 8종은 ①직접 또는 ②knowledge 경유로 이미 통과한다(`common-output-storage-and-path-management` ①=1·②=16, `common-beyond-mediocre-output` ①=3·②=16 등 1차 실측 유지 — 델타가 ②를 건드리지 않았다). 즉 좁아진 ③ 때문에 새로 FAIL하는 스킬은 없다 |
| RV-202 / RV-207 / RV-208 | 🟡 Minor + ⚪ Nit ×2 | **✅ 보류 타당** | 아래 "보류 판단 검토" 참조 |

## RV-203 기각 독립 판정 — 절반만 수용

PM 판단을 신뢰하지 않고 저장소를 직접 훑었다. 결과는 **결론(이번 라운드에 고치지 않는다)은 타당하나, 제시된 근거는 부분적으로 부정확**하다.

**(1) 기각의 핵심 논거는 옳다.** "설치사마다 저장소 배치가 달라 `malgn-agent/` 접두어를 박는 것이 오히려 틀린다" — `agents/trainer.md:40`이 편집 대상을 "malgn-agent 소스(조직의 git clone, 맑은소프트 한정으로는 이 저장소 `claude-plugins` 자체) 안의 `agents/<name>.md`"로 정의한다. 다른 조직에서는 clone 루트가 곧 malgn-agent이므로 접두어를 박으면 그쪽이 깨진다. **원 지적이 제시한 첫 번째 대안은 기각이 맞다.**

**(2) 근거로 든 "관례"는 실제로는 혼재다.** 실측(제품 본문의 grep 명령 인용 전수 + `malgn-agent/` 접두어 경로 전수):

| 표기 | 사이트 수 | 실물 |
|---|---|---|
| 접두어 없음 (`agents/ skills/ knowledge/`) | **7** | `evaluator.md:62`(4개 grep)·`:63`, `trainer.md:123`·`:124` — 전부 **grep 명령 안** |
| `malgn-agent/` 접두어 | **8** | `evaluator.md:42`·`:67`, `trainer.md:72`·`:87`·`:90`·`:115`·`:134`·`:154` — 전부 **경로 참조** |

즉 "본문 전반이 상대경로 관례"가 아니라 **grep 명령은 bare, 경로 참조는 접두어**로 갈려 있다. 더 결정적으로 `evaluator.md:42`는 한 문장 안에서 자기모순한다 — "판정 대상은 항상 `malgn-agent/<카테고리>/<name>` 형태의 평면 경로 하나뿐입니다(예: `agents/pm.md`, `skills/<name>/SKILL.md`, …)". 형식을 접두어로 선언하고 예시는 접두어 없이 든다.

**(3) 원 지적의 두 번째 대안은 답변되지 않았다.** RV-203은 "`malgn-agent/`를 박거나 **'플러그인 루트 기준' 한 마디 병기**"를 제안했다. 기각 사유는 첫 번째만 반박한다. 기준 디렉토리를 한 번도 명시하지 않은 상태는 남아 있고, 그래서 이 저장소에서 그 grep을 그대로 치면 지금도 실패한다.

**판정**: RV-203을 **⚪ Nit으로 강등하고 백로그**로 넘긴다. 재서술된 형태는 "접두어를 박아라"가 아니라 **"grep 명령의 기준 디렉토리가 malgn-agent 소스 루트임을 한 곳에서 못박고, `evaluator.md:42`의 선언-예시 불일치를 함께 맞춘다"**이다. 단독 1줄 수정으로는 표기만 갈리므로 동종 15개 사이트를 한 라운드에 묶어야 한다 → 이번 델타의 병합 차단 사유 아님.

## 보류 판단 검토 (RV-202 / RV-207 / RV-208)

**결론: 세 건 모두 보류가 타당하다.** 변경 동결 원칙의 판정 기준("지금 무엇이 깨져 있는가"에 답할 수 있으면 결함)에 셋 다 답하지 못한다 — 현재 `validate-agent-assets.mjs`는 ERROR 0으로 exit 0이라 체이닝이 실제로 무엇을 가리고 있지 않고(실행 확인), `check-doc-drift-spec.mjs`는 PASS 4·FAIL 0으로 목적을 달성한다.

다만 보류 사유 중 하나는 **절반만 성립**한다. "`check-drift-spec` 단독 별칭이 완화책으로 있다"는 근거인데, 그 별칭은 `package.json:9`에만 존재하고 **`CLAUDE.md` Commands 절과 `docs/README.md` 어디에도 문서화돼 있지 않다**(두 파일에서 `check-drift-spec` 검색 → 0건). CLAUDE.md Commands 절은 `check-assets`·`check-docs` 둘만 안내한다. 즉 완화책이 존재는 하나 **읽는 사람에게 도달하지 않는다** — 세션이 package.json을 열어봐야 알 수 있다. 마스킹 기제 자체는 실재를 확인했다(`scripts/validate-agent-assets.mjs:1294` `if (errors.length > 0) process.exit(1)`).

→ **권고(선택, 저비용)**: 백로그로 넘기되, `CLAUDE.md` Commands 절에 별칭 1줄을 추가하는 것은 PM 직접 편집 허용 구역이고 제품 본문을 건드리지 않는다. 지금 깨진 것이 없으므로 강제하지 않는다.

## 신규 지적 (이번 델타가 유입한 것)

| # | 심각도 | 관점 | 위치 | 확인방법 | 문제 | 개선안 |
|---|---|---|---|---|---|---|
| NV-301 | 🟡 Minor | 실행가능성 | `malgn-agent/agents/trainer.md:123` | 새·옛 패턴을 저장소에서 실제 실행해 히트 수 비교 + hub 도구 스키마 목록 대조 | 새 문면이 "도구를 하나씩 열거하면 그 목록 자체가 하나를 빠뜨리므로, **hub 기록 도구가 공유하는 접미어**로 잡는다"고 단정하며 `grep -rnE '_record\|issue_resolve'`를 제시한다. 그런데 접미어를 공유하지 **않는** hub 기록 도구가 남아 있다 — `wbs_add`/`wbs_bulk_add`/`wbs_update`(**7사이트**: `agents/pm.md`, `skills/project-orchestration/SKILL.md`)와 `project_bootstrap`(**8사이트**). `pm.md:29`는 "`wbs_add`/`wbs_bulk_add`로 단계를 먼저 등록한다"는 전형적인 **기록 주체 서술**인데 이 패턴에 안 걸린다. 옛 열거형은 "이건 목록이다"라고 보여서 독자가 확장하지만, 새 문면은 catch-all로 읽혀 **확장을 멈추게 한다** — 그래서 불완전한 catch-all이 완전한 열거보다 나쁠 수 있다 | 패턴에 `wbs_`를 더해 넓히거나, "이 패턴이 덮는 범위는 기록계 도구이며 WBS 계열은 별도로 본다"를 1구절 병기. **방향 자체는 옳다** — 실측상 옛 4개 열거는 111히트, 새 패턴은 130히트로 `issue_record` **29사이트**를 새로 잡는다(열거의 누락이 실증됐다). 완결성만 부족하다 |
| NV-302 | ⚪ Nit | 프로세스 | 델타 전반 (`trainer.md:123`) | PM 위임의 "직전 라운드 지적의 처리 결과" 표 ↔ 델타 diff 전량 대조 | 위임 처리표는 trainer.md에 대해 "RV-203 **기각**"만 적었으나, 실제 델타는 **같은 줄을 다른 이유로 재작성**했다(도구 열거 → 접미어 패턴). 커밋 메시지에는 적혀 있으니 은폐는 아니지만, 재검토 스코프를 "표에 적힌 6건"으로 좁힌 리뷰어라면 이 변경을 통째로 지나친다. Sensitive 델타에서 **미고지 변경 1건**은 검증망에 구멍을 낸다(이번엔 diff 전량을 읽어 잡았다) | 재검토 위임서의 처리표를 변경 통계와 대조해 표에 없는 파일·줄이 있으면 "그 외 변경" 행으로 명시. 비용 0 |
| NV-303 | ⚪ Nit | 실행가능성 | `malgn-agent/agents/evaluator.md:62` | `grep -rl common-product-principles-reference agents/*.md` → `agents/evaluator.md`, `agents/trainer.md` (2건) | 새로 추가된 예시가 스킬명 `common-product-principles-reference`를 **evaluator.md 본문에 실었다**. 그 결과 같은 줄이 지시하는 `grep -rl <스킬명> agents/*.md`를 그 스킬에 대해 실행하면 **2**가 나오는데, 바로 옆 문장은 "스킬명으로는 **한 자리뿐**"이라고 적혀 있다. 판정 도구가 자기 자신의 예시를 세는 자기참조 오염이다(결론은 안 바뀐다 — 경로③이 13이라 어느 쪽이든 PASS) | 예시 문장에 "(판정 시 판정 문서 자신의 예시 자리는 세지 않는다)" 반 줄을 붙이거나, 예시에서 스킬명을 서술형으로 풀어 쓴다 |
| NV-304 | ⚪ Nit | 운영(리뷰어 자산) | `docs/reviewer/personas/INDEX.md` | `ls -la` → **101,187 B**, "최근 재사용" 열 원문 확인 | "최근 재사용" 열이 라운드마다 `(직전: … (직전: … ))`로 **중첩 누적**돼 한 셀이 수백 바이트가 됐고 괄호 짝도 맞지 않는다. 이 파일은 **모든 재검토 착수 시 Read 대상**(재사용 판정 게이트)이라 비용이 상시화된다. 제품 본문이 아니고 이번 델타의 결함도 아니므로 병합과 무관 | "최근 재사용"은 **최근 1건만** 남기고 과거는 각 페르소나 파일의 "적용 이력"에 이미 있으니 중복 보관하지 않는다. 리뷰어 자신의 자산 정리이므로 백로그 |

## 발산형 관점 — 1,936 B 단일 체크박스는 정당한가

`agents/evaluator.md:62`는 이번 델타로 **1,359 B → 1,936 B(+577 B)**가 됐다(`sed -n '62p' | wc -c` 실측). 체크박스 하나가 2 KB에 육박한다.

**"커졌으니 반려"는 하지 않는다** — 늘어난 577 B는 전부 경로③의 검색어 도출 규칙과 실동작 예시이고, 그것이 없던 1차 때 나는 검색어를 즉흥으로 잡아야 했으며 그 사실을 재현성 리스크로 지적했다. 이번에는 문면대로 검색어를 뽑아 같은 결론에 도달했다(위 RV-206b). **재현성이라는 성능 이득이 실증됐으므로 이 증분은 정당하다.**

동시에 **"성능에 기여하니 여기 둬도 된다"도 아니다.** 이 절차는 evaluator가 스킬 신설·개명 심사 회차에만 쓰는데 evaluator 호출 100%에 로드된다. 이는 1차 보고서의 **Rethink 1(산식을 조건부 비용 자리로 이동)이 이미 다루는 문제**이고, 이번 증분은 그 제언의 근거를 강화할 뿐 새로운 구조 문제가 아니다. **새 백로그 항목을 만들지 않는다** — 근거 약한 항목이 쌓이면 우선순위 판단이 흐려진다는 저장소 규율에 따라, Rethink 1에 "이후 라운드에서 +577 B가 더해져 해당 항목이 1,936 B가 됐다"는 실측만 덧붙이면 충분하다.

## 이력·식별자 유입 검사 (제품 본문)

델타의 `malgn-agent/` 하위 `+` 라인 전량을 `8자리 hex | 26자 ULID | RV-nnn | YYYY-MM-DD | N차 | vX.Y` 패턴으로 스캔 → **히트 0건**. 날짜 도장·라운드 번호·커밋 언급·조회 불가 식별자 유입 없음.
(`docs/refactor/evaluator-budget-rationale.md`의 "작성" 줄에 `backlog-fix-round1` 트랙명이 추가됐으나 `docs/`는 제품 본문이 아니므로 규칙 대상 밖이다.)

## 상시 비용 판단

| 파일 | 크기 | 이번 델타 증분 | 판단 |
|---|---|---|---|
| `agents/evaluator.md` | **24,131 B** | +824 B | **정당** — 경로③ 재현성 확보(실증) + `previousScore` 귀속 명시. 예산 사유서가 실측치로 변호하며 검사기 INFO |
| `agents/trainer.md` | **28,757 B** | +약 100 B | **정당하나 미완** — 패턴 교체로 `issue_record` 29사이트를 새로 덮는다(순증 효과). 다만 NV-301대로 WBS 계열이 여전히 빠진다. 사유서 없는 `BUDGET_UNJUSTIFIED` WARN은 이번 델타가 만든 것이 아니라 베이스부터 있던 상태(WARN 18 동일) |

## 검사 실행 기록 (전부 직접 실행)

| 검사 | 결과 |
|---|---|
| `node scripts/validate-agent-assets.mjs` (브랜치) | `ERROR 0 · WARN 18 · INFO 2`, exit 0 |
| 같은 스크립트 (베이스 `43f2040` 트리 스냅샷을 스크래치패드에 풀어 실행) | `ERROR 0 · WARN 18 · INFO 2` — **브랜치와 동일, 베이스 복귀 확인** |
| `node malgn-agent/hooks/doc-drift.mjs` (`check-docs`) | agents 21 · skills 38 · knowledge 44 전건 일치, PM 관리구역 정상, "문서가 코드와 일치" |
| `node scripts/check-doc-drift-spec.mjs` | `PASS 4 · FAIL 0` |
| `agent_get_context(trainer, 3)` 라이브 호출 | 응답 실물로 `latestScore.overallScore` 확인 |

## 생략한 관점 / 미확인 (정직 보고 — 2차)

- **화면 캡처: 생략함** — 델타가 전부 `.md`·`.mjs`라 렌더링 화면이 없다.
- **`43f2040..c58d8e8` 구간: 재훑지 않음** — 위임 지시대로 델타(`c58d8e8..10fed1c`)만 봤다. 1차 판정을 그대로 승계한다.
- **`scoreHistory` 원소의 전체 필드: 부분 확인** — 조회된 원소는 `{overallScore, createdAt}` 2필드였다. 이력이 1건뿐인 계정이라 다건일 때의 정렬 순서(최신순 보장 여부)는 확인하지 못했다 — `latestScore`를 쓰라는 새 문면은 이 미확인에 의존하지 않으므로 판정에 영향 없다.
- **페르소나 '적용 이력' append·INDEX 갱신: 하지 않음** — "리뷰 보고서 갱신만 허용" 제약 준수(위 재사용 판정 절에 고지).
- **실행 액션 없음** — 제품·스크립트 파일 수정 0건, 커밋·push·PR 생성 **하지 않았다.** 이 보고서 append만 했다. 베이스 비교용 트리 스냅샷은 스크래치패드에만 풀었고 워크트리·브랜치는 건드리지 않았다.

## PM에게 권고 (2차)

1. **병합 차단 사유: 없음.** 1차 Major(RV-201)가 실측·실행으로 닫혔고, 검사기가 베이스와 동일 상태(`ERROR 0 · WARN 18 · INFO 2`)로 복귀했다. Sensitive 등급이므로 사람 승인은 여전히 전제조건이다.
2. **같은 라운드 처리 권고: 없음.** 신규 지적 중 Major 이상 0건. NV-301(Minor)은 지금 무언가를 깨뜨리지 않고 방향은 개선이므로 다음 trainer 라운드로 넘겨도 된다.
3. **trainer 반환 대상 (다음 라운드)**: NV-301(grep 패턴에 WBS 계열 포함 또는 범위 한정 병기), NV-303(예시의 자기참조 계수), 그리고 강등된 RV-203(기준 디렉토리 명시 — 동종 15사이트 묶음).
4. **PM 직접 처리 가능(선택)**: `CLAUDE.md` Commands 절에 `check-drift-spec` 별칭 1줄 — RV-202 보류의 완화책이 실제로 읽히게 한다.
5. **백로그 유지**: 1차 Rethink 3건 + RV-202/207/208 + `hooks/doc-drift.mjs:392`의 식별자 + NV-304(INDEX.md 중첩 열 정리). **Rethink 1에는 "해당 항목이 1,936 B로 커졌다"는 이번 실측을 덧붙이면 우선순위 판단에 도움이 된다.**
6. **프로세스 1건(NV-302)**: 재검토 위임서의 처리표를 변경 통계와 대조해 "표에 없는 변경"을 명시하는 습관. 이번에 trainer.md 재작성 1건이 표 밖에 있었다.
