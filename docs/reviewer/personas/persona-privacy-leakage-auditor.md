...
- 2026-09-01 / target_id `golden-task-benchmark-20260901` / 1차(최초, Exploration 풀패널) — 역할개념 수준 재사용(대상을 개인정보에서 "공개 채널로 무엇이 새는가"로 확장 적용). **핵심 발견 2건**: (a) `gh repo view` 실측 결과 이 저장소는 **PUBLIC** — 벤더의 비공개 내부 게이트 우회 플래그 이름이 커밋 대상 2파일 4곳에 평문으로 들어가 공개 게시된다. 설계문서 R1은 "정식 승인 전 임시 조치"라는 사내 관점만 다루고 공개 게시 리스크는 미언급이며, 사용자 승인 범위가 여기까지인지 불명(M-4). (b) `pnpm run eval:golden`이 아무 가드 없이 package.json에 등록되고 `docs/README.md`에 광고됨 — 이 저장소는 `/plugin marketplace add` 클론 대상이라 임의의 직원이 오발동 시 약 $16·1시간이 **그 직원 계정에** 청구되고, 조직이 닫아둔 게이트를 그 직원 이름으로 우회하며, R2대로 자식 세션에 그 직원의 claude.ai 커넥터가 등록된다. R2는 "외부 케이스를 우리가 돌리는 것"만 다루고 "우리 케이스를 남이 돌리는 것"은 다루지 않는다(M-5).
