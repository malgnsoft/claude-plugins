...
- 2026-08-26 / target_id `audit-r2-hooks-20260825` / 2차(축소 재검증, Sensitive, 발산형) — 역할개념 수준 재사용(§5 참고파일은 직전 라운드 고정). 1차 R1(가드 건강도를 드리프트와 분리한 축)·R2(체크별 `optionalHost`)는 백로그 확정 상태로 **여전히 유효**하며 이번 diff가 둘 중 어느 것도 선점·차단하지 않음을 확인(주석만 바뀌어 코드 구조 무변경). 신규 R3: RV-101이라는 결함 자체의 발생 원인이 "**사양 정본이 실행 코드 파일의 헤더 주석에 있다**"는 배치에 있다 — `SKILL.md:78`이 지정한 정본이 `doc-drift.mjs` 상단 50줄 주석이라, 코드를 바꾼 커밋이 같은 파일 위쪽 주석을 함께 고치지 않으면 사양과 구현이 한 커밋 안에서 갈라진다(이번이 실제 사례). 이번 수정은 그 갈라짐을 메웠을 뿐 재발 기제는 그대로 두었고, 헤더는 5줄 더 길어졌다(50줄 → 55줄, 파일 열 때마다 상시 읽히는 비용).
