...
- 2026-09-01 / target_id `golden-task-benchmark-20260901` / 1차(최초, Exploration 풀패널) — 역할개념 수준 재사용. 대상: 설계문서 §0 「하네스 사실관계」 8행 + §5 근거 서술을 `claude plugin eval --help` 원문 및 `docs/anthropic/` 미러와 대조. 확인: `--publish-report`가 이미 기본값 · `--threshold` 기본 1.0 · `--runs` 기본 `case.runs ?? 3` · `--eval-dir`이 "below the plugin" · 우선순위 `--eval-dir > 매니페스트 > evals/` · 서브에이전트 멘션 `@agent-<plugin>:<agent>`(`docs/anthropic/agents/sub-agents.md:792` 원문 일치) — **전건 참**. 신규 지적: (a) `--ablation` 기본이 `with-without`(2× runs)임을 --help가 명시하고 내 프로브 실행이 실증 — 래퍼가 `none`으로 잠근 것은 회당 비용을 절반으로 지킨 조치(잘된 점), (b) `--max-cost-usd` 상한 히트 시 하네스가 **유료 그레이더를 건너뛰고 free 그레이더만 채점**한다는 --help 조항을 §5가 누락 → 비용 초과가 곧 가짜 회귀 경보로 나타나는 연쇄 미기술(M-6), (c) `docs/anthropic/`에 plugin eval 미러가 0건이라 §0 표가 이 저장소의 유일한 사양 기록인데 근거 trace가 소멸(m-1).
- 2026-09-01 / target_id `golden-eval-stage2-cases-20260901` / 1차(최초, Exploration 풀패널) — 역할개념 수준 재사용(§5 참고파일은 하네스 사양 미러가 0건이라 "원문 미확인" 처리 규칙만 적용). 신설 그레이더가 새로 의존하는 하네스 사양 가정 3건을 판정: ①`target: files`가 줄바꿈 결합 문자열이다 → 파일럿 `artifacts-under-docs`가 실제 통과한 기록으로 간접 확인, ②정규식 컴파일에 `m` 플래그가 없다 → **원문 미확인**. 이번에 처음 등장한 부정 선행탐색 그레이더의 극성이 이 가정에만 의존하며, `m`이면 fail-open으로 뒤집힘을 node 실측으로 재현, ③`input_match`가 부분문자열 매칭이다 → 미확인이며, 부분문자열이면 `security`는 산출물 파일명에 포함돼 하드 게이트가 헛통과.
- 2026-09-01 / target_id `sidechain-instream-nesting-count-20260901` / 1차(최초, Sensitive 풀패널) — 역할개념 수준 재사용. 이번 라운드는 대조 정본이 공식문서가 아니라 **하네스 자신이 남긴 `*.meta.json`**이었다(Task/sidechain 내부 동작은 공개 문서에 그 수준으로 없다). 검증 대상 주장: "사이드체인 라인이 Task/Agent를 발행하면 그게 곧 중첩 위임". `meta.json.toolUseId`를 로그의 Task/Agent `tool_use.id`에 전수 조인 → matched 746 / 양방향 고아 0, `spawnDepth >= 2` ⟺ 발행 라인 `isSidechain === true` **746/746 일치, 불일치 0건**으로 전제 성립 확인. 보조: assistant 112,423줄에서 `isSidechain===true` ⟺ `subagents/` 하위 파일 예외 0건. 한계: 코퍼스 최대 `spawnDepth`가 2라 3단 이상 깊이는 구조적 논증만 가능(간선 단위 계수·평면 저장·전부 sidechain).

- 2026-09-11 / target_id `stop-hook-additional-context-20260911` / 1차(최초, Sensitive 풀패널) — 역할개념 수준 재사용(§5 참고파일은 `docs/anthropic/hooks/hooks.md`로 교체). `stop-mcp-reminder.cjs` 변경이 새로 단정한 하네스 사양 4건을 미러 원문과 대조: ①additionalContext가 모델 컨텍스트에 들어간다 → `hooks.md:972`·`:990`·`:2542` 일치, ②`hookSpecificOutput`은 `hookEventName` 필수이고 Stop 값은 `"Stop"` → `:919`·예시 `:2555-2562` 일치, ③`decision:"block"`은 훅 에러로 표시된다 → `:2542`·`:2553` 일치, ④루프 보호가 block과 동일(stop_hook_active + 8회 상한) → `:2553`·`:2474` 일치. **4건 전원 원문 일치, 지어낸 사양 0건.** 신규 지적: `:1003`이 "명령형 out-of-band 지시보다 사실 서술을 쓰라"고 권고하는데 리마인더 본문은 명령형(…확인하라/무시하라)이며, 이 권고는 본문이 실제로 모델에 닿게 된 이번 변경부터 처음 적용 대상이 된다(라이브 1회 관측에서는 프롬프트 인젝션 방어 미발동).
- target_id `wbs-issue-closure-skill-20260915` / 1차(최초, Sensitive 풀패널) — 역할개념 수준 재사용(대조 정본은 공식문서가 아니라 **세션에 로드된 malgnai-hub MCP 도구 스키마 원문**과 **라이브 응답**). 신설 스킬이 단정한 hub 호출 사양 8건 전수 대조: `wbs_list(includeDone)`·`wbs_update(status/progress/completedDate)`·`issue_resolve(projectId/issueId/result)`·`issue_record(idempotencyKey 필수)`·`project_get_context(sections/limit)`·`project_search_history(types=['issue'])` — **발명된 파라미터 0건, 이름·타입 전건 일치**. 라이브 호출로 추가 확인: 기본 `wbs_list(projectId)` 응답에 `summary{total,planned,inProgress,done,delayed}` 5필드가 스킬 서술과 정확히 같은 이름으로 실재하고, `items` 60건 중 `status:"done"` **0건**(= 기본값이 done을 items에서 제외한다는 서술 참) · 그룹 노드에 `bucket`/`computedProgress` 실재(예: `status:"planned"` + `computedProgress:91` + `bucket:"in_progress"`) · `project_get_context(sections=['issues'])`는 `status:"open"`만 반환하고 `id`와 `description` 전문을 함께 실어 3건에 약 5KB(단계 3의 토큰 경고가 실측과 부합). **다만 같은 라이브 응답이 단계 1의 인과 서술(:27)을 반증한다** — `summary`는 `includeDone` 없이도 `done:149`를 그대로 주므로, 스킬이 지시한 대로 summary로 판정하는 한 `includeDone:true`를 빼도 오독이 나지 않는다(M-1). 미검증으로 남긴 것: `idempotencyKey` 재사용 시 서버 멱등 동작, 이미 닫힌 이슈에 대한 `issue_resolve` 반환 코드(스키마 서술 없음), `STATUS_DONE_LEAF_ONLY` 실발생(에러를 유발하는 쓰기 호출은 리뷰 권한 밖).
