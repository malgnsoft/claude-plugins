 ...
 * docs/anthropic/hooks/hooks.md:852 "Can block? No", :1001 "Context only... No blocking or
 * decision control"). 다만 "규율이 통째로 사라진 채 세션이 정상으로 보이는" 상태는 조용히 넘기지
 * 않고 systemMessage 로 사람에게 알린다(PM 블록 모드, 아래).
 *
 ...
 * "값 하나"당 10,000자에서 캡되고, 넘으면 에러가 아니라 파일로 저장되고 미리보기+경로로 조용히
 * 대체된다. STATUS.md 는 (위 상한 로직에도 불구하고) ASCII 위주면 12,000바이트가 10,000문자를
 * 넘을 수 있고, PM 블록과 같은 문자열에 이어붙이면 STATUS.md 쪽 분량이 PM 블록의 생존(잘림 여부)을
 * 좌우하게 된다 — 규율이 상태 메모 크기에 인질로 잡히는 구조는 받아들일 수 없다.
 * docs/anthropic/hooks/hooks.md:971 이 "여러 훅이 같은 이벤트에 additionalContext 를 반환하면
 * Claude 가 값을 모두 받는다"고 명시하므로, PM 블록을 **별도 hooks.json 항목**(별도 프로세스,
 * 별도 stdout JSON)으로 분리하면 두 값이 각자 독립적으로 10,000자 캡을 받는다 — PM 블록 자신의
