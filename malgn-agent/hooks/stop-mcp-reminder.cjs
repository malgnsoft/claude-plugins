...
//   - Edit/Write/Agent/Workflow 등 실질 편집 도구를 썼는데 기록은 안 했다 → 리마인더 필요
//   - Read/Grep/Bash 등 조회성 도구만 썼거나 아무 도구도 안 썼다 → 트리비얼로 간주(스킵)
// 파싱 실패/불확실하면 항상 안전한 쪽(리마인더 표시)으로 폴백한다 — 기록 누락 방지가
// 노이즈 감소보다 우선.

const fs = require("fs");

// 작업신호는 **편집 도구**로 좁힌다 — Bash는 여기 넣지 않는다.
// 실제 세션 트랜스크립트를 재생해 보면 이 훅이 발동한 턴의 상당수가 Bash 하나만 신호였고,
// 그중 다시 상당수가 `git status`·`ls`·`cat`·`grep` 같은 순수 조회였다. 리마인더가
// systemMessage(사용자 표시)에 머물던 동안에는 이 오탐이 무해했지만, 지금은
// additionalContext로 실제 모델 턴을 하나 더 만들기 때문에 오탐 하나가 곧 비용이 된다.
//
// 대가(의도적으로 받아들인 것): `git commit`·배포 명령처럼 **Bash만으로** 기록할 만한 일을
// 한 턴은 이제 리마인더를 받지 못한다. 그 턴은 사람·에이전트의 자기판단에 맡긴다 —
// 오탐을 줄이는 편이 전체 비용에서 낫다는 판단이다. Bash 명령 문자열을 뜯어 조회성인지
// 가려내는 방식은 이 훅이 감당할 범위를 넘어선다(명령 하나에 조회와 변경이 섞인다).
//
// PowerShell은 남긴다 — Windows 전용 세션이 이 도구로만 작업하면 신호가 통째로 사라져
// 무음 실패가 되기 때문이다. Bash와 같은 오탐 소지가 있는지는 따로 실측해야 판단할 수 있다.
const WRITE_TOOL_RE = /^(Edit|Write|NotebookEdit|PowerShell|Agent|Workflow)$/;
...
process.stdin.on("end", () => {
  ...
  // 문구는 **사실 진술형**으로 쓴다(명령형 금지). 훅이 주입하는 텍스트를 "시스템 명령"처럼
  // 쓰면 모델의 프롬프트 인젝션 방어가 발동해, 컨텍스트로 쓰이는 대신 사용자에게 그대로
  // 노출돼 버린다 — 모델에게 닿게 하려고 additionalContext로 보내는 이 훅에서는 그 순간
  // 수정 자체가 무의미해진다. 그래서 "무엇을 하라"가 아니라 "지금 상태가 이렇다 / 이 도구들이
  // 이런 용도다"로 적고, 판단은 모델에게 맡긴다.
  const reason = [
    "[malgnai-hub 기록 점검] 이번 턴에는 파일 편집·서브에이전트 실행 같은 편집 도구가 쓰였고, malgnai-hub 기록 도구는 쓰이지 않았다.",
    "이 환경에서 기록에 쓰는 도구는 넷이다:",
    "- " + T + "decision_record — 방향·정책·기술선택 등 주요 결정을 남기는 도구",
    "- " + T + "issue_record — 막힌 것·장애물·버그를 남기는 도구",
    "- " + T + "issue_resolve — 그 이슈가 해소된 것을 확인했을 때 닫는 도구",
    "- " + T + "work_record — 의미 있는 작업의 진행·완료·막힘을 남기는 도구. nextAction 값이 채워져 있으면 다음 세션이 그 지점에서 이어진다",
    "기록할 내용이 없는 턴(단순 조회·질문 응답 등)도 있다. STATUS.md 갱신은 이 도구들과 별개 경로이며 서로를 대신하지 않는다.",
  ].join("\n");

  // 리마인더의 수신자는 **모델**이다 — 기록 여부를 자가 점검해야 하는 주체가 모델이기 때문이다.
  // systemMessage는 사용자 터미널에만 뜨고 모델의 대화 컨텍스트에는 전혀 들어가지 않으므로,
  // 그것만 반환하면 사람 눈에는 보이는데 정작 행동해야 할 모델은 내용을 한 번도 못 받는다.
  // Stop 이벤트에서 모델에게 닿는 필드는 hookSpecificOutput.additionalContext다.
  //
  // decision:"block"은 쓰지 않는다 — 같은 "대화 계속" 효과를 내지만 훅 에러로 표시돼
  // "정상 작동 중인 안내"가 장애처럼 보인다. additionalContext는 비에러 피드백으로
  // 트랜스크립트에 `Stop hook feedback`으로 표시된다.
  //
  // 루프 보호: additionalContext도 block과 동일한 보호(stop_hook_active 입력 + 연속 8회 상한)를
  // 받는다. 이 파일 앞부분의 stop_hook_active 조기 종료 때문에, 리마인더로 이어진 다음 Stop에서는
  // 아무것도 출력하지 않는다 — 한 체인당 계속은 최대 1회라 8회 상한에 닿을 수 없다.
  // 추가 턴이 생기는 것 자체가 문제인 무인 헤드리스 워커는 위 MALGNAI_HEADLESS_WORKER 예외로
  // 이 지점에 도달하기 전에 빠진다.
  //
  // systemMessage는 사용자 가시성 유지를 위해 병행 반환하되 본문 전체가 아니라 한 줄 알림으로
  // 둔다 — 본문은 이미 트랜스크립트에 피드백으로 표시되므로, 같은 ~600B를 터미널 경고로 한 번 더
  // 찍으면 중복 노이즈만 늘어난다. 두 필드는 각각 10,000자 상한을 받으며 둘 다 그보다 훨씬 짧다.
  //
  ...
  process.stdout.write(
    JSON.stringify({
      ...
      systemMessage: "[malgnai-hub 기록 점검] 이번 작업의 기록 여부를 확인하도록 안내했다.",
    })
  );
});
