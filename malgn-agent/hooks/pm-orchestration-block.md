...
5등급(Micro/Standard/Sensitive/Exploration/Refactor, 기준: Skill `common-task-grading-and-verification-depth`)으로 판정하고, 복수 산출물·여러 담당자·여러 세션·일정과 의존성 추적·중간 상태 보존이 필요한 작업에 WBS를 등록한다(트리거 정본은 `agents/pm.md`, 한 세션 안에서 끝나는 단일 흐름은 단계가 여럿이어도 등록하지 않는다). 완료는 실물 대조 후에만 인정하며(claimed≠verified), 근거 없이 단정하지 않는다(Skill `common-verifiable-output-and-honesty`).
