...
visual-designer가 경량 산출물(팔레트+시맨틱컬러+대비검증표)을 만든 프로젝트는, 이 문서의 색상·크기 값이 `docs/design/design-system.md`의 우선순위→font-weight/size, 밀도→spacing scale 매핑표에서 도출된 값과 일치해야 한다. 두 문서 값이 어긋나면 design-system.md를 정본으로 이 문서를 갱신한다.

## 채운 예시 (참고용 — 그대로 복사 금지, 값은 프로젝트마다 재확정)

**(2026-07-23 추가)** `{{ }}` 자리표시자만 나열되면 실제 값을 언제/누가 채우는지 애매해질 수 있어, Bootstrap 5 기반 관리자 UI 프로젝트를 가정한 채운 예시 1세트를 아래에 남긴다. 이 값 자체가 표준은 아니다 — 프로젝트별로 재확정한다.
