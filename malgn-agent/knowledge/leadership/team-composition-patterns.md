...
**병렬 시 지켜야 할 것**
- 같은 저장소에 동시에 쓰는 병렬 위임은 `isolation:"worktree"`로 격리한다(격리 없이 진행하면 브랜치가 얽힌다). 파일을 쓰지 않는 조회·리서치까지 강제하지는 않는다 — 다만 격리가 병렬에만 걸리는 요구는 아니다. 순차로 진행하는 작업까지 포함한 일반 규율은 Skill `project-orchestration` §4.3에서 확인한다
