...
```
- ux-designer는 화면이 하나 신설되거나 기존 화면의 기능이 변경/추가되면 기본 투입(예외는 Micro 등급과, 화면 신설 없이 기존 패턴·동선을 그대로 재사용하는 Fast Path 변경 둘뿐 — Skill `common-task-grading-and-verification-depth`). visual-designer는 ux-designer 산출물의 판단에 따라 조건부 투입.
...
다단계 사슬(A→B→C…) 위임 시 Supervisor가 매 단계 제어권을 회수하는 "경로 릴레이" 원칙 — 사고사례·근거는 `coo-rule-rationale.md` 항목4가 권위자다. 실행 절차 요약은 `agents/pm.md`의 "경로 릴레이 순차" 항목 참조.
...
## 순차 실행 vs 병렬 실행 — 판단 기준

기준은 **산출물 의존성**이다. 일률적으로 순차만 쓰지 않는다(대기 시간이 배로 늘고, 서로 독립인 판정끼리 앞선 결론에 끌려간다).

**순차로 돌린다 — 앞 산출물이 뒤 입력일 때**
- 앞 에이전트의 산출물을 뒤 에이전트가 읽어야 하는 사슬(A→B→C)
- 이때도 인계 주체는 항상 PM이다(경로 릴레이 순차: A 호출 → A가 파일 저장·경로 반환 → PM이 제어권 회수 → B 호출)

**병렬로 돌린다 — 한쪽 결과가 다른 쪽 입력이 아닐 때**
- 전역 자산 승격 트랙의 evaluator·reviewer는 **항상 병렬**이다. 같은 초안을 다른 관점으로 보므로 순서를 강제할 이유가 없고, 엇갈린 두 결과의 차이 자체가 판단 재료다
- 서로 독립인 작업 슬라이스

**병렬 시 지켜야 할 것**
- 같은 저장소에 동시에 쓰는 병렬 위임은 `isolation:"worktree"`로 격리한다(격리 없이 진행하면 브랜치가 얽힌다). 파일을 쓰지 않는 조회·리서치까지 강제하지는 않는다
- 같은 목업/시드 엔티티를 여러 슬라이스가 참조하면 정본 데이터셋을 먼저 확정해 전달하거나, 완료 시점에 파일 간 교차대조를 거친다

> 위임·추적·검증의 실행 절차는 Skill `project-orchestration`을 따른다.
