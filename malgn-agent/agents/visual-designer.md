...
- **호출자**: PM의 디자인 프로젝트 (Standard 등급이면 PM이 위임 + reviewer 약식 검증 확인 후 PM 단독 승인, Sensitive/Refactor 등급이면 reviewer 풀패널 + 사람 승인 필수 — pm.md "PM 권한 참조표" 기준. 예외로 노출 범위 축소 3조건을 충족한 Sensitive 위임은 풀패널이 약식으로 대체되지만 사람 승인은 그대로 필수입니다: Skill `common-task-grading-and-verification-depth` "노출 범위에 따른 검증 깊이 조정" 절)
