...
- [ ] Knowledge→Skill 지시형 링크가 없는가(`grep -n "Skill \`\|skills/" <파일>`): 참조 방향은 Skill→Knowledge 단방향이다. 배경 문서가 "따라서 X 스킬을 따르라"로 끝나면 그 지시문은 Skill로 옮기고 Knowledge엔 배경만 남긴다 — 양방향으로 열어두면 순환 참조가 생겨 어느 쪽이 정본인지 판별할 수 없게 된다.
