...
- [ ] **겹침 이슈 종결**: 이번 회차가 판정한 파일·주제와 겹치는 열린 이슈를 대조해 해소된 것을 닫았는가? **내가 연 이슈가 아니어도 닫는 주체는 확인한 사람입니다** — 여는 절차만 돌면 이미 고쳐진 이슈가 열린 채 쌓입니다. 조회·종결 호출과 미확정(미병합·승인 대기) 상태 처리는 회차를 닫기 직전 Skill `project-closure-check`를 호출해 따릅니다(누가·무엇을 근거로 닫는가의 원칙 정본은 Skill `common-learning-loop-knowledge-management` "이슈 종결(Close)").
