...
| **이슈 종결**(`issue_resolve` — 열린 이슈가 해소된 것을 실물 대조로 확인했을 때. 해소가 아직 확정되지 않았으면 닫지 말고 지목) | ✅ 확인한 쪽이 닫는다 | ✅ 확인한 쪽이 닫는다 | ✅ 확인한 쪽이 닫는다 |

**이슈 종결만은 역할이 아니라 "누가 확인했는가"로 정해진다** — 위 표의 다른 행과 달리 셋 다 ✅인 이유다. 내가 연 이슈가 아니어도, 이번 작업의 본래 목적이 아니어도, 해소를 확인한 쪽이 그 자리에서 닫는다. 미루면 아무도 돌아오지 않아 이미 고쳐진 이슈가 열린 채 쌓이고, 다음 세션이 그 목록을 믿고 끝난 일에 착수한다. **다만 "확인했다"와 "확정됐다"는 다르다** — 해소 근거가 아직 기본 브랜치에 들어가지 않았으면 닫지 않고 근거와 함께 종결 후보로 지목해 확정한 쪽에 넘긴다(`issue_resolve`는 되돌릴 수 없다). Trainer는 커밋까지만 하고 병합은 하지 않으므로 이 지목이 기본 경로다. 절차·부분 해소 처리는 Skill `common-learning-loop-knowledge-management` "이슈 종결(Close)"이, 확정 판정과 지목 방법은 Skill `project-closure-check` 단계 4-1이 정본이다.
...
- [ ] **PM 규율 두 벌 정합 확인**: `agents/pm.md`의 역할 경계·위임·승인 규칙을 고쳤다면 `hooks/pm-orchestration-block.md`(사람과 대화하는 메인 세션에 매 세션 주입되는 같은 규칙)도 함께 열어 정합을 확인했는가? 한쪽만 고치면 같은 조직 안에서 메인 세션과 서브에이전트가 서로 다른 규칙으로 움직인다.
