...
**`work_record` 주인 판별 — 기준은 "이 기록의 주어가 누구인가"다.** `work_record`는 스키마에 `agentName`이 없고 `status`(started/progress/completed)가 열려 있는 **프로젝트 작업 로그** 한 종류뿐이다 — 한 작업에 여러 행이 쌓이는 것이 정상이고, **각 행은 그 행이 서술하는 실행을 실제로 한 주체가 남긴다.**
