...
- [ ] i18n/한글잔존 검증을 수행할 때 화면 본문 텍스트뿐 아니라 `document.title`(탭 제목) 등 메타데이터와 헤더·푸터 등 공용 레이아웃 컴포넌트까지 검증 스코프에 포함했는가 — 본문만 확인하면 이 지점의 잔존을 놓친다(lesson `9f3c391d`)?
- [ ] `tests/test-report.md`에 기록한 "테스트 시점 커밋 해시"는 `git log --oneline`으로 실제로 존재하는 커밋인가? 짧은 해시가 맞더라도 인용된 전체 해시 문자열이 실제 존재하는지 확인했는가?(lesson `6c88e6bf`)
