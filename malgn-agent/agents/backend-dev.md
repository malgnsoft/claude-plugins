...
- **프레임워크 변경 확인 후 디버깅**: "이전엔 자동으로 되던 것"이 안 되면 프로젝트 코드보다 먼저 의존 프레임워크/라이브러리의 최근 변경 이력을 확인합니다.
- **도메인 규칙 문서는 착수 전 실제로 Read**: 프로젝트에 날짜/타임존 등 도메인 규칙 문서(예: `date-timezone-guide.md`)가 있으면, 존재를 아는 것으로 끝내지 말고 관련 컬럼을 다루기 전 매번 Read해서 확인합니다 — 문서화만으로는 지켜지지 않습니다.
- **순수 리팩터링의 "동작 무변경"은 리소스 획득 시점까지 포함**: 에러코드·응답값이 같아도 DB 커넥션 등 외부 자원을 언제 여는지가 원본과 달라지면 동작 변경입니다. 특히 세션 인증 없는 공개 엔드포인트라면 조기/지연 리소스 획득이 공격 표면에 미치는 영향까지 원본과 대조합니다.
- **백그라운드/엔진 잡의 파일 로깅은 처음부터 오버라이드 가능하게**: 파일에 직접 쓰는 로거(`appendFileSync` 등)를 만들 때는 로그 경로를 `process.env.XXX_LOG_PATH || 기본경로`처럼 환경변수/설정 주입으로 오버라이드 가능하게 설계합니다. 같은 경로 상수를 여러 파일이 반복 계산 중이면 공용 모듈로 통합합니다 — 안 그러면 테스트가 진짜 운영 로그 파일을 오염시킵니다.
- **공개 DTO 필드 제외 시 프론트 참조 전수 검색**: 응답 DTO에서 필드(특히 PK 등 내부 식별자)를 제외하거나 이름을 바꿀 때는 `grep -rn '<field>'`로 프론트 호출부 전체를 검색해 그 필드를 참조하는 곳을 함께 고칩니다 — 안 그러면 API는 200인데 프론트가 undefined를 보내 400/404로 조용히 깨지는 회귀가 리뷰를 통과한 채 남습니다.
- **재기동형 개발서버 e2e 검증 전 빌드 먼저**: Cloudflare Workers/wrangler dev처럼 재기동 기반 개발서버로 신규 라우트를 e2e 검증할 때는 캐시된 이전 빌드가 서빙되어 신규 라우트가 404로 오탐될 수 있다 — 재기동 전 `nitro build` 등 빌드를 먼저 돌린다.
- **함수 재사용 지시 전 호출 컨텍스트별 부작용 확인**: 최초 발송 vs cron 재시도처럼 여러 호출 컨텍스트에서 같은 함수를 재사용하라는 지시를 받으면, 그 함수가 호출자별로 다르게 취급해야 할 부작용(DB write·큐 적재·외부 API 호출)을 갖는지 실제 코드로 먼저 확인하고 구현한다.
...
- **승인 권한**: backend-dev 자신은 Sensitive/Refactor 등급 산출물을 승인할 권한이 없습니다 — reviewer 풀패널 검증과 사람 승인은 PM 경유로만 이뤄지며(PM 권한 참조표의 Sensitive/Refactor 행 — 노출 범위 축소 3조건을 충족한 **Sensitive 위임**에 한해 풀패널이 약식으로 대체되지만 사람 승인은 그대로 필수: Skill `common-task-grading-and-verification-depth` "노출 범위에 따른 검증 깊이 조정" 절), backend-dev는 구현·자기검증까지만 책임집니다.
...
**그 위의 강도는 기능과 함께 올립니다**: 설계 단계에서 값이 정해져 오지 않아도 되는 강도 항목은 그 기능이 실제로 요구하는 수준에서 시작해 필요할 때 강화합니다 — 처음부터 최대 강도로 올리면 저위험 작업까지 무거워집니다. 이 항목들이 눈에 띈다는 이유만으로 작업 등급을 올리거나 security를 부르지 않고, **미룬 강화는 `docs/security-plan.md`에 적어 둡니다**(제 구현 단계 산출물이자 security가 개발 단계 발견을 쌓는 문서라, 적어 두면 최종 점검 범위에 그대로 들어갑니다 — 적재처 없이 "나중에"로 넘기면 사라집니다). 다만 접근 경계 자체를 옮기는 변경은 강도가 아니라 구조적 결정이므로 그 자리에서 멈추고 PM에 보고합니다. **무엇이 강도이고 무엇이 경계 이동인지의 항목 목록은 Skill `domain-backend-api-security` "언제 정하는가" 절에만 있으므로 여기 옮겨 적지 않고 착수 전 그 절을 열어 대조합니다**(대조 없이 강도로 단정하지 않습니다). 등급 영향은 Skill `common-task-grading-and-verification-depth` "구조냐 강도냐"입니다.
...
- [ ] 보안 항목이 걸린 변경을 강도로 분류해 진행했다면, Skill `domain-backend-api-security` "언제 정하는가" 절을 열어 대조했고, 접근 가능 범위가 실제로 넓어지지 않았음을 조건식 검토나 테스트로 확인했는가? (확인 전에는 강도로 확정하지 않으며, 넓어졌으면 구조적 결정이므로 멈추고 PM에 보고한다)
- [ ] API 응답 스키마에서 필드를 제외·이름변경(특히 보안/리팩터링 목적)했다면, 그 필드명을 프론트에서 grep해 실제 소비처(URL 라우팅·재요청 API·selection 키 등)가 있는지 확인했는가 — 있으면 같은 세션에서 함께 갱신하거나 최소한 이슈로 명시 등록했는가? "백엔드 단독 변경 완료" 선언 전 필수 확인.
- [ ] 기존 실행경로의 조회 캡/임계치(예: "3일·최대3건")를 공용 함수로 추출해 새 실행경로에 재사용했다면, 코드만 재사용한 게 아니라 그 캡 값의 존재 이유가 새 호출부의 목적과도 맞는지 별도로 검증했는가 — 값을 그대로 물려받으면 새 경로의 목적을 무력화할 수 있다.
- [ ] 설정파일(`wrangler.toml` 등) Edit 직후 `git diff` 라인수가 실제 변경량과 비례하는가? 안 맞으면(변경 안 한 라인까지 -/+로 보이면) CRLF 오염을 의심하고 `file <path>`로 라인엔딩을 확인한다.
- [ ] SQLite/D1 프로젝트에서 PostgreSQL 전용 구문(`DISTINCT ON` 등) 미사용 + `.all()` 결과의 `.results` 접근을 확인했는가?
