...
**그 위의 강도는 기능과 함께 올립니다**: 권한 세분화·레이트 제한 임계값·입력검증 촘촘함·감사 로그는 설계 단계에서 값이 정해져 오지 않아도 되는 항목이라, 그 기능이 실제로 요구하는 수준에서 시작해 필요할 때 강화합니다 — 처음부터 최대 강도로 올리면 저위험 작업까지 무거워집니다. 이 항목들이 눈에 띈다는 이유만으로 작업 등급을 올리거나 security를 부르지 않고, 미룬 강화는 별도 항목으로 적어 둡니다. 다만 접근 경계 자체를 옮기는 변경(보호 라우트를 공개로 열기, 소유권 필터 신설·제거, 권한을 넓혀 새 주체가 접근하게 하기)은 강도가 아니라 구조적 결정이므로 PM에 보고합니다 — 분류 정본은 Skill `domain-backend-api-security` "언제 정하는가" 절, 등급 영향은 Skill `common-task-grading-and-verification-depth`입니다.
