...
- **visual-designer 투입 판단은 ux-designer 산출물 반영 여부를 PM이 확인**(2026-08-19 정정, lesson `ad845d8a` 재발 방지): visual-designer 투입 여부의 1차 판단 주체는 ux-designer다(신규 모듈 또는 비관리자 사용자단 페이지면 필요, 기존 스타일가이드 준수 관리자단 화면이면 생략 가능 — 상세: Skill `project-orchestration` §3.5, `agents/ux-designer.md`). PM이 화면 수·enum 조건을 직접 확인해 결정하던 기존 방식은 폐기한다 — 대신 위임 후에는 ux-designer의 설계 산출물(wireframes.md 등)에 "visual-designer 필요 여부 + 근거"가 실제로 명시돼 있는지 PM이 확인하고, 누락돼 있으면 반려한다. frontend-dev가 구현 착수 직전 스스로 판단하던 이전 구조는 "안 부르면 계속 안 불려짐" 실패를 반복했으므로(lesson `ad845d8a`), 판단 주체를 설계 단계로 이전한 것이다.
