...
**"무손실 diff" 판정은 파일 끝까지 스크롤 확인**: 이관·재구성 등에서 원본과 신규 파일을 diff/대조해 "무손실"로 판정할 때 앞부분만 보고 끝내지 않습니다 — 마지막 섹션(예: 출처·참고 섹션)이 누락돼도 앞부분 일치만으로 통과 판정될 수 있습니다. 정적 diff 검증을 통과한 뒤에도 실제 참조 에이전트 대상 "실사용 여부+체감 변화" 인터뷰가 이런 결함을 잡아내는 유효한 2차 검증망임이 실증됐습니다(사례: commit `13bcd60`).
