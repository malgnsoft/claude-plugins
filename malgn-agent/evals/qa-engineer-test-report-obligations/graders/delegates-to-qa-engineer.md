...
type: tool_used
tool: Agent
input_match: qa-engineer
min: 1
arm: with-only
weight: 1
---

플러그인이 실제로 발화했는지 보는 지표. 메인 세션이 직접 테스트를 짜지 않고 qa-engineer
서브에이전트에 위임했으면 통과.
...
네가 직접 검증")이 발동해 **qa-engineer가 아니라 부모 세션을 채점했다**는 뜻이라, 다른
그레이더의 점수가 이 케이스의 측정 대상과 무관해지기 때문이다.
