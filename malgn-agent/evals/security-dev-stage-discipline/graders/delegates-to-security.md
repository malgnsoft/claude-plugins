...
확정이다** — `scripts/run-golden-eval-manual.mjs:236`이 도구 입력 객체 전체를 JSON 문자열로 만들어
대소문자 무시 부분일치로 보므로, 인용문이 하위 프롬프트에 실리기만 해도 통과한다. **공식 하네스에서도
성립하는지는 미확인이다** — 아래 남은 불확실성대로 입력의 어느 부분을 대조하는지에 달렸다. 어느
쪽이든 이 값이 확실히 줄이는 것은 낱말 `security`가 여기저기 섞여 생기는 우연한 일치다.
