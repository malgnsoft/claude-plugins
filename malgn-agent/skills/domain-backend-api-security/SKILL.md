...
- 응답으로 **무엇을 내보내는가**의 경계 — 기존 응답에 없던 데이터를 새로 싣는 변경 중, 그 데이터가 개인정보·자격증명·타 주체 소유 정보인 경우. 저장된 데이터를 노출하는 순간 "무엇에 접근할 수 있는가"가 넓어진다 (§1 IDOR·§2-4)
...
- 이미 그 요청자에게 열려 있는 범위 안에서의 응답 필드 추가·표현 변경 (§2-4) — 새로 싣는 데이터가 개인정보·자격증명·타 주체 소유 정보면 (a)다
...
- **분류는 의도가 아니라 결과로 한다.** "좁히려던" 변경이라도 조건을 잘못 결합해(AND로 좁히려다 OR가 되는 식) 실제 접근 가능 범위가 넓어지면 (a)다. 좁아졌다는 것이 조건식 검토나 테스트로 확인되기 전에는 (b)로 확정하지 않는다 — 확인 없이 (b)로 내리면 리뷰도 권한 확인도 함께 빠진 채로 넓어진 경계가 나간다.
- **강도 조절 대상이 아닌 최소 요건**이 따로 있다: 파라미터화 쿼리(§4), fail-closed(키·설정이 없으면 통과가 아니라 거부), 비밀번호·토큰을 로그·응답에 싣지 않기. 이것들은 "나중에 강화"의 대상이 아니라 처음부터 지키는 것이며, **지키는 것 자체는** 작업 등급을 올리는 요소도 아니다 — 구현을 제대로 하는 일의 일부다. 반대로 **이 최소 요건을 깨는 방향의 변경은 (b)가 아니라 (a)다**: 설정·키가 없을 때 거부하던 것을 통과로 바꾸기, 검증을 건너뛰는 우회 플래그·환경변수 추가, 파라미터화를 문자열 조립으로 되돌리기. 전부 "누가 무엇에 접근할 수 있는가"를 넓히므로 Sensitive다.
...
**미루기로 한 강도 항목은 `docs/security-plan.md`에 적는다.** 이 문서가 개발 단계 보안 발견의 적재처이자 backend-dev의 구현 단계 산출물이라, 미룬 항목이 security의 최종 점검 범위에 그대로 들어간다. 적재처를 정하지 않고 "나중에"로 넘기면 미룬 강화는 아무 데도 남지 않고 사라진다.

아래 표는 **등급 축(무엇을 건드리는가)만으로** 판정한 예시다. 사용자 규모·배포 범위는 등급을 정하는 축이 아니므로 근거로 쓰지 않는다("사내용이라서"는 인증·권한 검증을 건너뛰는 만능 면제 사유가 된다).

| 작업 | 걸리는 항목 | 판정 |
|---|---|---|
| 조회 API 응답에 **이미 그 요청자에게 열려 있는 범위 안의** 필드 하나 추가 | (b)만 — 접근 경계도 노출 범위도 그대로 | Standard(Fast Path 5조건 충족 시 Fast Path). 새로 싣는 값이 개인정보·자격증명·타 주체 소유 정보면 노출 경계가 넓어지는 것이라 (a) → Sensitive |
| 이미 admin 전용인 라우트에 "자기 부서 레코드만" 조건을 더해 권한을 좁힘 | (b) 권한 세분화 | Standard — 단 실제로 좁아졌음이 조건식·테스트로 확인된 뒤에만. 확인 결과 넓어졌거나 권한을 넓혀 새 주체가 접근하게 되면 경계 이동이라 (a) → Sensitive |
| `WHERE id = ?`만 쓰던 조회에 소유권 필터를 처음 도입 | (a) 접근 경계 신설 | Sensitive |
| 결제 웹훅 수신 라우트 신설 | (a) 인증 게이트·소유권 매핑·멱등 처리를 새로 정함 + 결제 도메인 | Sensitive |
| 이미 인증·권한 게이트 뒤에 있는 라우트의 처리 결과를 감사 로그에 항목으로 더 남김 | (b)만 | Standard |
| 키·설정이 없을 때 거부하던 검증을 통과로 바꾸거나 우회 플래그를 추가 | (a) 최소 요건을 깨는 방향 | Sensitive |
