...
- 체크리스트 보강
- 근거는 문장으로만 적는다 — 이 규칙이 왜 필요한지(어떤 실패가 반복되는지)를 현재형 한 문장으로 남기고, 기록 id·커밋 해시는 붙이지 않으며 날짜 도장도 남기지 않는다. 계기가 된 프로젝트·시점·기록 id는 malgnai-hub 학습 이력에만 두고 MD 본문에는 옮기지 않는다(`agents/trainer.md` 핵심 원칙의 식별자·이력 금지)
