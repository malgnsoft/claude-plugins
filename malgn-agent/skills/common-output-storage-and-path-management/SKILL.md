...
### 1-1. 번들 스크립트 실행 규약 (`${CLAUDE_PLUGIN_ROOT}`)

플러그인 `bin/`의 스크립트를 실행하라는 지시는 **항상 아래 한 가지 형태**로만 적고, 그 줄을 **그대로 복사해 실행**한다.

```bash
node ${CLAUDE_PLUGIN_ROOT}/bin/<스크립트>.mjs [인자...]
```

- **이 변수는 스킬·에이전트 본문이 모델에 도달하기 전에 플러그인 절대경로로 치환된다**(2026-08-23 실측: 에이전트 MD 본문·스킬 본문 양쪽에서 치환 확인). 즉 위 줄은 받아본 시점에 이미 완성된 명령이다 — 경로를 추측하거나 조립하지 않는다.
- **셸은 이 변수를 모른다.** Bash 툴 세션에서 `echo ${CLAUDE_PLUGIN_ROOT}`는 **빈 문자열**이라, 변수를 직접 타이핑하면 `/bin/...`을 찾다가 `MODULE_NOT_FOUND`로 실패한다. 셸에서 변수를 풀려고 시도하지 말고, 본문에 채워져 도착한 절대경로를 쓴다.
- **치환은 스킬·에이전트 본문에서만 일어난다**(같은 실측). `knowledge/`·`templates/`·`bin/` 안의 파일을 Read로 열면 변수가 문자 그대로 보인다 — 그런 문서에서 실행이 필요하면 그 명령을 소유한 스킬을 열어 거기 채워져 나오는 경로를 쓴다.
- **맨 명령어로 실행하지 않는다.** 플러그인 `bin/`이 Bash 툴의 PATH에 등재되긴 하지만 번들 스크립트 일부에 실행 비트가 없어 `permission denied`로 끝난다. `capture.mjs ...`가 아니라 `node ${CLAUDE_PLUGIN_ROOT}/bin/capture.mjs ...`로 적는다.
