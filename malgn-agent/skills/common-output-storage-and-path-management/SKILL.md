...
- **`${...}` 형태만 치환된다.** 맨이름 `CLAUDE_PLUGIN_ROOT`는 그대로 남는다(관측) — 이 문단이 변수 이름을 적을 수 있는 이유다. 거꾸로 제목·산문에 `${...}` 형태를 쓰면 그 자리에 긴 절대경로가 통째로 박혀 문장이 망가진다. 변수를 *지칭*만 할 때는 맨이름으로 쓰고, 실행 커맨드에만 `${...}` 형태를 쓴다.
