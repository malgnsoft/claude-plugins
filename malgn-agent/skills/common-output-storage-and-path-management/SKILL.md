...
- **어디서 치환되는가 — 관측으로 확정한 목록.** 치환은 *디렉터리*의 성질이 아니라 **하네스가 그 파일을 플러그인 컴포넌트로 로드할 때** 일어나는 일이다. 그래서 "이 폴더는 되고 저 폴더는 안 된다"가 아니라 "어떻게 읽히는가"로 갈린다.
...
- **큰따옴표는 선택이 아니다.** 치환값은 리터럴 절대경로라, 사용자 홈 경로에 공백이 있으면(`/Users/Gil Dong/...`) 무따옴표는 단어분리로 `MODULE_NOT_FOUND`가 난다(공백 포함 경로에서 재현됨: 무따옴표 실패 / 따옴표 성공). `hooks/hooks.json`이 같은 이유로 따옴표를 쓰고 있으니 제품 안에서 형태를 갈리게 두지 않는다.
...
- **예외 — 플러그인 밖으로 복사돼 나가는 코드.** 사용자 프로젝트로 복사되는 템플릿 코드(`templates/e2e-template/auth.setup.js` 등)는 복사된 순간 플러그인 컴포넌트가 아니라 이 변수가 풀리지 않는다. 그런 코드가 자기 파일 위치를 기준으로 런타임에 경로를 계산하는 것(`import.meta.url` 등)은 정상이며 이 규약 위반이 아니다 — CLAUDE_PLUGIN_ROOT 형태로 "고치면" 오히려 깨진다.

### 1-2. 플러그인 자원 참조 규약 (knowledge/ 등)

에이전트·스킬 본문에서 플러그인 자원(`knowledge/`의 학습 자료·스타일가이드·로고)을 **열라고 지시할 때**는 `${CLAUDE_PLUGIN_ROOT}/knowledge/<도메인>/<파일>` 한 형태로 적는다. 에이전트는 사용자 프로젝트를 cwd로 돌기 때문에 맨 상대경로 `knowledge/...`는 그 프로젝트 안에서 찾다가 실패하고(서브에이전트가 본문 그대로 Read하면 `File does not exist`로 실패한다), "이 플러그인의"처럼 산문으로 위치를 가리켜도 마찬가지다 — 사람에게는 뜻이 통하지만 Read 도구에는 경로가 아니다.
