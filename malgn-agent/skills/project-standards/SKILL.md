...
- **없으면 → in-place 스탬프 후 2단계로 간다.**
  ...
  - **스탬프를 돌렸다고 배선이 갖춰졌다고 보지 않는다** — CLAUDE.md가 이미 있으면 스캐폴더가 그 파일을 통째로 건너뛰므로, 그 안의 배치·크기가 표준에 맞는지는 확인되지 않은 채 남는다. 확인은 2단계에서 한다.
...
2. **문서 지도** — `docs/README.md`가 있는지 확인한다. 있으면 내용을 다시 쓰지 않는다. 없으면 스캐폴더(`bin/new-project.mjs`)가 스탬프하는 지도 형식이 뼈대의 정본이므로 그 형식으로 만든다 — 진입점 선언 + "먼저 읽을 것"(`STATUS.md` → `CLAUDE.md` → malgnai-hub `project_get_context`) + `docs/` 하위 폴더 목록. (§4는 이 지도를 **쓰는** 원칙이지 만들 뼈대가 아니다.)
3. **malgnai-hub 연동** — `STATUS.md` 상단 YAML frontmatter의 `project_id`를 본다.
...
새로 만든 파일 / 건너뛴(기존 유지) 파일 / 환경 점검 3항목 각각의 결과(정상 · 교정함 · 사용자 확인 필요) / malgnai-hub 연동 결과를 요약 보고한다.
...
- [ ] "초기화 해줘"를 처리했다면, STATUS.md 유무와 무관하게 환경 점검 3항목(CLAUDE.md 배치·크기 / `docs/README.md` / malgnai-hub 연동)을 모두 확인했는가? — STATUS.md 하나만 보고 "이미 초기화됨"으로 끝내지 않는다(§8)
