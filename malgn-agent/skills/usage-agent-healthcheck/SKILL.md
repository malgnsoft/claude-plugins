...
- **대상**: `token-usage-diagnosis`와 마찬가지로 특정 소수 에이전트 전용이 아니다. PM이 Micro 등급으로 직접 처리하거나(전문 에이전트 위임 불요), 세션 중인 어느 에이전트든 사용자가 물으면 바로 실행한다.
