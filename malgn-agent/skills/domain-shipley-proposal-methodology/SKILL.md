---
name: domain-shipley-proposal-methodology
...
제안서를 **단계별로 다른 색의 팀이 다른 관점에서** 검토한다. 자기가 쓴 글은 객관적으로 못 본다는 전제. reviewer 에이전트가 평가위원 페르소나 패널로 이를 수행한다(패널 구성·심각도 표준·보고서 형식은 Skill `reviewer-persona-panel-standard`).
...
5. **제출 전 실격방지 체크** (반드시) — `korea-public-procurement.md` / Skill `domain-compliance-matrix-template`.
