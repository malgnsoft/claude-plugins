...
| Standard | 일반 기능·API·버그수정 | 위임 | PM 단독 승인(사람 승인 불요) | 약식(페르소나 1~2, 발산형 생략 가능) | 정상흐름 + 주요 예외 1개 | standard — 완성시점 1회 + 경량체크리스트 4항목 | - |
...
| Exploration | 원인분석·구조파악, 무엇을 고칠지 모름 | 위임(시간·산출물 제한) | 승인 대상 아님(변경 산출물 없음, PM은 결론 채택 여부만 판단) | 결론 검증만 | 없음 | trivial — 결론 검증만이라 캡처 불필요 | - |
