...
**정본 경계**: 여기는 종결의 **원칙**(누가 · 무엇을 근거로 · 부분 해소)이, Skill `project-closure-check`는 라운드 종결 시점의 **조회·대조·종결 호출**(WBS·미확정 처리 포함)이 정본이다 — 서로 복제하지 않는다.
...
- **라운드를 마칠 때 겹침을 재점검한다.** 이번 라운드가 손댄 파일·주제와 겹치는 열린 이슈를 대조해 해소됐으면 닫는다 — 새로 여는 것만 하면 목록이 한 방향으로만 늘어난다. 조회 도구·파라미터는 위 경계대로 Skill `project-closure-check`를 호출해 따른다(도구를 잘못 고르면 지시를 지켜도 "닫을 것이 없다"로 오독된다).
...
- [ ] 라운드 종료? → Skill `project-closure-check`로 WBS·이슈를 재점검 후 종결
