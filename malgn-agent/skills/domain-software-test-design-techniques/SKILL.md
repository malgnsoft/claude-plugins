---
name: domain-software-test-design-techniques
description: 경계값분석·동등분할·상태전이 등 범용 테스트 설계기법 + Vitest 단위/통합/Mock 패턴, Playwright E2E, 테스트 보고서 형식, "401/400만 찍는 테스트는 빈 깡통" 안티패턴과 인가·비즈니스규칙·상태전이까지 내려가는 커버리지 처방. qa-engineer/reviewer가 테스트를 설계·검토할 때 사용한다.
---
...
## 게이트의 환경 의존성 주의
route 테스트가 라이브 dev 서버·시드 DB 가동에 의존하면 게이트 자체가 환경 의존적이 된다 — 커버리지 100%여도 서버가 내려가 있으면 게이트가 조용히 붕괴한다(테스트가 전부 skip/에러로 빠지는데 "실패"로 잡히지 않는 구성이면 특히 위험). 가능하면 격리된 테스트 DB/모킹으로 외부 서버 의존을 줄이고, 불가피하게 라이브 서버에 의존한다면 서버 헬스체크를 테스트 스위트 진입 조건으로 명시한다(사례: PMS 종합 건강진단).
...
출처: 실서비스 테스트 코드 진단에서 얻은 교훈.

### 진단으로 발견한 실제 패턴 (안티패턴)
진단 대상의 `test/routes/*.test.js`는 12개 라우트 파일 전부가 **인증 없이 접근 → 401**, **필수 필드 누락 → 400** 두 가지만 반복 검증했다. `rbac.test.js`는 모든 보호 엔드포인트에 대해 401을 전수 검증(좋은 패턴)했지만, 그 외에는:
...
라우트를 거치지 않고 Service를 직접 호출하면 분기·경계값을 싸고 정밀하게 검증할 수 있다 (해시·검증 유틸 같은 순수 함수에는 대개 이런 단위 테스트가 이미 붙어 있다 — 같은 방식을 도메인 규칙을 담은 서비스로 확장하라).
