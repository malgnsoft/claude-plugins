...
## 게이트의 환경 의존성 주의
route 테스트가 라이브 dev 서버·시드 DB 가동에 의존하면 게이트 자체가 환경 의존적이 된다 — 커버리지 100%여도 서버가 내려가 있으면 게이트가 조용히 붕괴한다(테스트가 전부 skip/에러로 빠지는데 "실패"로 잡히지 않는 구성이면 특히 위험). 가능하면 격리된 테스트 DB/모킹으로 외부 서버 의존을 줄이고, 불가피하게 라이브 서버에 의존한다면 서버 헬스체크를 테스트 스위트 진입 조건으로 명시한다(사례: PMS 종합 건강진단, lesson `9e3c1c96`).
...
출처: coaching 프로젝트 테스트 진단 (2026-06-20, 모드 7). 실제 코드를 진단해 얻은 교훈.
