...
캡처는 결정론적 기능(스크립트 실행)이라 **별도 캡처 전문 에이전트를 두지 않는다** — reviewer(또는 frontend-dev)가 아래 스크립트를 직접 실행해 이미지를 모으고, 그 이미지를 Read로 보며 판단한다. "캡처 담당 에이전트에게 위임" 패턴은 쓰지 않는다.
...
- 검증이 권한 규칙에 막히면 우회하지 말고 정직 보고한다(Skill `common-permission-policy-compliance`).
