...
- **크리티컬 패스 모니터링**:
  ...
  - ⚠️ **필터를 생략한 `wbs_list(projectId)`는 전체 스냅샷이 아니다** — 기본값이 `items`에서 done을 빼므로 스크립트에 넘길 때는 `includeDone=true`를 명시한다. `includeDone`·`status`는 `summary`를 바꾸지 않으므로(걸러지는 것은 `items`뿐) 이 둘에 대해서는 `summary.total !== items.length`가 필터 탐지식으로 성립한다(스크립트도 이 식으로 탐지해 리포트 상단에 경고한다)
  - ⚠️ **`parentId`는 예외다 — `summary` 자체가 그 서브트리 집계로 좁혀진다.** 그래서 서브트리 조회는 `summary.total`과 `items.length`가 같아져 위 탐지식에 걸리지 않고, `summary.total`을 프로젝트 전체 건수로 읽으면 "전체가 몇 건뿐"이라는 그릇된 판정이 된다. 전체 스냅샷은 `parentId` 없이 뜬다(집계 기준의 정본: Skill `project-closure-check` 단계 1)
