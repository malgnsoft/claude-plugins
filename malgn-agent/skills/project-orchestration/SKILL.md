...
새 도메인 지식이 필요할 때 매번 전담 에이전트를 새로 만들면 팀이 과다해진다(§3 "과다팀 금지"와 같은 이유). PM은 아래 기준으로 "지식팩(knowledge 보강)"과 "전담 에이전트 신설" 중 하나를 고른다.

- **판정 기준(3조건 모두 충족 시에만 신설)**: ①**규제깊이** — 도메인 특유의 규정·법적 요건이 깊어 일반 knowledge로 뭉뚱그리기 어렵다. ②**반복오류** — 기존 담당 에이전트가 이 도메인에서 반복적으로 실수한 실적이 있다(1회성 실수가 아니라 패턴). ③**지속성** — 일회성 프로젝트가 아니라 앞으로도 계속 반복될 작업이다.
- **하나라도 미충족이면 지식팩으로 처리한다**: 기존 제너럴리스트 에이전트(architect/backend-dev 등)의 `knowledge/<도메인>/`를 trainer가 보강하는 쪽이 기본값이다. 전담 에이전트 신설은 예외이지 기본이 아니다.
- **신설을 판단했다면**: trainer에게 위임하기 전 위 3조건 판정 근거를 한 줄씩 명시해 위임 메시지에 포함한다 — evaluator가 승격 심사에서 같은 근거를 되짚을 수 있어야 한다.

**seedling 표기(실험적 신설 시 필수)**: 3조건을 충족해 신규 에이전트/스킬을 신설하더라도, 그 유용성이 아직 검증되지 않은 실험 단계라면 "seedling"으로 표기하고 명시적 일몰조건을 함께 적는다(예: "seedling — 3개월 내 재사용 없으면 폐기 검토"). PM은 trainer에게 신규 자산 초안 작성을 위임할 때 이 표기를 MD/SKILL.md 상단 또는 evaluator 위임 메시지에 관행적으로 포함하도록 권고한다 — 일몰조건 없이 신설된 자산은 아무도 정리 시점을 판단하지 못한 채 방치되기 쉽다.
...
5. **라운드를 닫기 전 열린 이슈를 재점검한다** — `project_get_context(projectId, sections=['issues'])`로 열린 이슈를 열거해 이번 라운드가 손댄 파일·주제와 겹치는 것을 고르고, 실물 대조로 해소된 것은 `issue_resolve`로 닫는다(부분 해소면 `result`에 해소분·잔여분을 적어 닫고 잔여만 새 이슈로 다시 연다 — 열린 이슈를 갱신하는 도구는 없다). 이슈를 여는 절차만 돌리고 이 단계를 빠뜨리면 목록이 한 방향으로만 늘어나, 다른 목적의 라운드가 부수적으로 고쳐놓은 항목이 열린 채 남아 다음 세션이 이미 끝난 일에 착수한다. 상세 규칙은 Skill `common-learning-loop-knowledge-management` "이슈 종결(Close)"이 정본이다.
