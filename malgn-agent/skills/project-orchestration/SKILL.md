...
- 본문이 이 파일에 그대로 있는 절(§2의 실행 커맨드·§3.5·§4.1·§4.2·§6·§7)은 따로 열 것이 없다. §5는 공통 절차가 여기 있고, 종결 점검(5항)은 별도 스킬로, 산출물 유형별 추가 항목은 절별 파일로 나가 있다. 매 위임·매 검증마다 걸리는 짧은 규율이거나, 플러그인 경로를 담고 있어 여기 있어야 하는 것들이다.
...
| 디자인 트랙(화면 신설/기존 화면 기능변경 시 기본 투입 — 예외는 Micro 등급과 화면 신설 없이 기존 패턴·동선을 재사용하는 Fast Path 변경뿐 — 예외 목록의 정본은 §3 본문 `team-composition.md`) | ux-designer | prd.md, requirements.md | `docs/design/ux-flow.md`, `docs/design/wireframes.md`, `docs/design/ia.md`(참조: `${CLAUDE_PLUGIN_ROOT}/knowledge/design/ux-design-guide.md`) — wireframes.md에 "visual-designer 필요 여부 + 근거" 명시 필수 |
...
- **노출 범위 3값(STAGE 1 planner 위임에는 필수)**: planner를 부르기 **전에** ①사용자 규모 ②배포 대상 ③취급 데이터 성격을 확정해 위임 메시지에 적는다. 브리핑에 없으면 추측하지 말고 `AskUserQuestion`으로 세 개를 한 번에 묶어 사용자에게 묻는다 — planner에게는 그 도구가 없어 스스로 물을 수 없고, PM이 대신 확인해주지 않으면 planner는 "3값 미확인"으로 반환하거나(기본 경로) 추정으로 요구사항을 큰 쪽에 맞춰 부풀린다. 확인이 끝내 불가능하면(이 실행에서 `AskUserQuestion`을 쓸 수 없는 경우 포함) "미확인 — 그대로 진행"임을 명시해 보내 planner가 전제로 적고 미결사항에 남기게 한다. 이 3값은 prd.md의 범위 경계 전제·NFR 수치·비교표 종류(대외=경쟁사 / 사내 전용=현행 방식)를 정하고, 이후 Sensitive 위임의 노출 범위 축소 근거로도 그대로 재사용된다(축 정의: Skill `common-task-grading-and-verification-depth` "노출 범위에 따른 검증 깊이 조정").
...
경로 릴레이 순차·슬라이스 위임·저장 경로 명시·`subagent_type` 명시 같은 전달 규약과, 위임 전에 실물로 대조해야 하는 것들(파일 스코프·API 응답 필드·호출자별 부작용·기존 자산 실재)·작업 격리(브랜치·워크트리)와 병합 완결. **실제 위임 프롬프트를 보내기 직전** `${CLAUDE_PLUGIN_ROOT}/skills/project-orchestration/delegation-transfer.md`를 Read한다.

## 5. 자기 검증 & 재작업
1. 위임 결과 검증 (claimed ≠ verified 원칙) — 각 에이전트 산출물이 지정된 경로에 실제로 존재하는가는 §3.5 산출물 지도를 기준으로 확인한다. 단 **planner가 노출 범위 3값 미확인을 사유로 파일 없이 반환한 것은 위임 실패도 산출물 미실재도 아니라 규정된 되묻기 경로다**(§4.1 "노출 범위 3값") — 재작업으로 다루지 말고 3값을 확정해 재위임한다.
2. 문제 발견 시 재지시 + 재검증 (최대 2회)
3. 중요 산출물은 직접 실물 검증 (PDF 페이지·UI·끝부터 끝까지)
4. 미검증 항목은 "미검증 + 사유"로 정직하게 보고.
5. **라운드를 닫기 직전 WBS·이슈를 재점검해 해소된 것을 닫는다** — 그 시점에 Skill `project-closure-check`를 호출해 따른다. 조회 도구와 파라미터를 잘못 고르면 종결된 항목이 목록에서 통째로 빠져 "닫을 것이 없다"로 오독되므로, 호출법과 그 이유는 여기 옮겨 적지 않는다(한쪽만 갱신되면 조용히 갈린다).
6. **격리해 진행한 브랜치의 병합 잔여 확인** — 라운드를 닫기 전 `git worktree list`·`git branch --no-merged`로 미병합·미승인으로 남은 브랜치가 있는지 본다. 남아 있으면 완료가 아니라 승인 대기다(§4.3 "병합 전에는 완료가 아니다").

7. **산출물 유형별 추가 검증** — 검증 대상이 ①설계 산출물 ②frontend-dev의 `Write` 전체 재작성 ③이전 세션의 "검증 완료(grep 0건)" 주장 인수 ④용어·i18n 치환 ⑤화면 캡처 보고 중 하나에 해당하면 `${CLAUDE_PLUGIN_ROOT}/skills/project-orchestration/verification-by-output-type.md`를 Read한다 — 유형마다 무엇을 어떻게 대조해야 통과인지가 그 파일에만 있다. 유형 이름만 보고 넘기면 그 유형이 막으려던 실패가 그대로 난다.
