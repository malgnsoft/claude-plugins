...
6. **산출물 유형별 추가 검증** — 검증 대상이 ①설계 산출물 ②frontend-dev의 `Write` 전체 재작성 ③이전 세션의 "검증 완료(grep 0건)" 주장 인수 ④용어·i18n 치환 ⑤화면 캡처 보고 중 하나에 해당하면 `${CLAUDE_PLUGIN_ROOT}/skills/project-orchestration/verification-by-output-type.md`를 Read한다 — 유형마다 무엇을 어떻게 대조해야 통과인지가 그 파일에만 있다. 유형 이름만 보고 넘기면 그 유형이 막으려던 실패가 그대로 난다.
