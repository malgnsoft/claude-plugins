...
## 백로그 기록·처리 원칙 (이 프로젝트의 중요원칙)
**STATUS.md는 제목+포인터만, 상세는 malgnai-hub `issue_record`에 — 그 "상세"는 재조사 없이 바로 처리할 수 있는 수준을 뜻한다.** 최근 실제로 백로그를 처리할 때마다 파일을 다시 읽고 grep을 다시 돌려 같은 조사를 반복했다 — 처음 발견한 시점에 아래를 적어두면 그 반복이 없어진다.
- **정확한 파일:줄**과 현재 문제되는 문구(인용) 또는 정확한 증상.
- **왜 문제인가** — 위반한 규칙·불변량, 실제 불일치 사례.
- **고친 뒤 참이어야 하는 상태**(요구사항·수용기준) — 문안 자체는 적지 않는다(그건 위임받은 전문가가 제안할 몫, 아래 [편집 권한 경계](#편집-권한-경계-반복적으로-무너져-명문화한다)의 "위임 지시서에는 설계를 쓰지 않는다"와 동일한 원칙).
- 같은 결함이 이미 다른 자리에서 고쳐진 **선례**가 있다면 그 위치(참고 문안·패턴)까지 적는다.
- **왜 지금 반영하지 않고 백로그로 미루는지**(아래 기준 참조) — 이게 없으면 다음 세션이 "왜 안 고쳤지"부터 다시 판단해야 한다.

**발견한 결함의 기본값은 "바로 처리"이지 "백로그 등재"가 아니다.** 아래를 모두 만족하면 새 백로그 항목을 만들지 않고 그 라운드 안에서 검증까지 마친다:
- 결함이 명백하다(설계 판단이 갈리지 않는다) — 위 [변경 동결 원칙]의 "결함" 기준을 충족.
- 지금 진행 중인 위임·검증 파이프라인이 이미 열어본 파일이거나, 반영 비용이 국소적이다(다른 작업의 리스크·크기를 유의미하게 키우지 않는다).
- 편집 권한 경계·Git 브랜치 원칙을 그대로 지킬 수 있다 — trainer 위임이 필요하면 위임하되, 새 라운드로 미루지 않고 같은 세션에서 이어서 처리한다.

백로그로 남기는 것은 구조변경·개선(변경동결 원칙상 보류 대상)이거나, 지금 처리하면 진행 중인 작업의 스코프·리스크가 커지는 경우로 한정한다. **이유**: 발견 즉시 백로그로 넘기고 나중에 별도 라운드로 몰아 처리하면, 그 라운드 자체를 진행하다 또 새 결함이 발견돼 백로그가 줄지 않고 오히려 늘어나는 순환이 반복된다.
...
- `malgn-agent/` — 마켓플레이스의 핵심이자 유일한 플러그인.
  ...
  - `skills/` 40종 — 명명은 참조 에이전트 수 기준(`common-*` 전역 상시비용 / `domain-*` 도메인 / 무접두어 단일 참조)
...
- `scripts/` — 저장소 전용 검사(배포되지 않음). `validate-agent-assets.mjs`(`pnpm run check-assets`) · `check-docs.mjs`(`pnpm run check-docs`의 진입점 — 위 Architecture의 자산 개수 표기를 실물과 대조하고 어긋나면 exit 1) · `check-install-reachability.mjs`(`check-links`) · `check-status-size.mjs`(`check-status`) · `sync-mcp-tool-usage-map.mjs`(`check-mcp-tools`) · `sync-anthropic-docs.mjs`(`sync-docs`) · `run-golden-eval.mjs`(`eval:golden` — 골든 태스크 벤치마크 실행기. **유료·수동 전용**이고 `GOLDEN_EVAL_I_ACCEPT_COST=1` 옵트인이 있어야 돈다. 설계: `docs/architecture/golden-task-benchmark.md`)
