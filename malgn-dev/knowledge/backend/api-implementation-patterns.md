...
inline 핸들러에 비즈니스 로직을 다 넣는 위 예시(`api-implementation-patterns` 상단)는 작은 앱엔 괜찮지만, 규모가 커지면 라우트가 비대해지고 테스트가 어렵다. coaching은 **얇은 라우트(입력 정규화·인가) + 클래스 기반 서비스(비즈니스 로직)**로 깔끔히 분리했다.
...
coaching의 `authService.register`는 `cc_users UPDATE` + `cc_invite_tokens UPDATE`를 **각각 실행**한다(트랜잭션 없음). 첫 쿼리 성공 후 둘째에서 실패하면 데이터가 어긋난다. **여러 행을 함께 바꾸는 작업은 `BEGIN/COMMIT`(pg) 또는 `db.batch`(D1)로 원자화**해야 한다 — backend-dev의 "트랜잭션 경계" 의무. happy path만 보면 놓치기 쉬운 부분.

```javascript
const client = await this.getClient();
try {
  await client.query('BEGIN');
  await client.query('UPDATE cc_users SET ... WHERE id = $1', [uid]);
  await client.query('UPDATE cc_invite_tokens SET status = $1 WHERE id = $2', ['accepted', iid]);
...
- **① Route → Service → DB** (coaching): 서비스 클래스가 비즈니스 로직과 SQL을 함께 가진다. 위 "Route → Service 계층 분리" 절이 이 형태다. DB 접근 지점이 적고 단일 진입(HTTP)만 있을 때 충분하다.
- **② Route → Service → DAO → DB** (malgnai): **DB 접근을 DAO 계층으로 분리**한다. DAO가 있으면 **SQL은 DAO에만 두고, Service/Route는 비즈니스 로직·입력정규화·응답 조립만** 담당한다. (malgnai는 라우트가 얇아 Service 단계 없이 Route → DAO → DB로 바로 가는 곳도 많다 — 핵심은 *SQL이 DAO 밖으로 새지 않는 것*이다.)

### 판단 기준: 언제 DAO를 두나
- 같은 테이블 쿼리를 **여러 진입점**(HTTP API + MCP 도구 + 배치/동기화 스크립트)이 공유 → **DAO 필수**.
- 쿼리가 한 곳에서만 쓰이고 앱이 작다 → ① 서비스에 인라인해도 무방. 나중에 진입점이 늘면 ②로 추출.

### DAO 계층의 이점 (malgnai 코드 근거)
- **쿼리 중앙화·재사용**: malgnai의 `server/mcp/tools.js`가 `ProjectsDao`/`TasksDao`/`ActivitiesDao`/`AgentsDao`/`FeedbacksDao`를 **API 라우트와 동일하게 import해 재사용**한다. MCP 도구와 HTTP API가 같은 쿼리·같은 멱등성 보장을 공유 → SQL을 두 번 쓰지 않는다. (CLAUDE.md "MCP tools는 기존 DAO 재활용"의 실체)
...
```javascript
// server/dao/projects.js (malgnai 실제 형태)
...
**F. 멀티테넌시 필수 규약 (performance `anti-patterns.md` — 실서비스 금지 패턴).**
...
출처: performance 프로젝트 실서비스 Hono 코드 진단(모드 7). performance는 malgnai와 거의 동일한 Hono 스택이되 **DAO 없이 `Route → Service → DB`(①형)** 으로 운영 중이다. 설계자는 performance를 만들며 "다음 프로젝트(malgnai)는 Service가 아니라 DAO로 가야겠다"고 결론지었다. 이 진화의 "왜"를 실제 코드 근거로 박제한다.
...
**한계 1 — 같은 쿼리가 여러 service에 흩어진다 (재사용 불가).** performance의 사용자 조회 SQL(`users LEFT JOIN departments LEFT JOIN teams ... WHERE u.email/u.id AND site_id`)은 `authService.login`(L66)과 `authService.refresh`(L323)에 **거의 동일하게 두 번** 복붙돼 있다. user_roles 조회(`SELECT role FROM user_roles WHERE user_id AND site_id`)도 두 곳(L97, L346)에 중복. DAO가 있었다면 `UsersDao.findForAuth(email, siteId)` 하나로 끝났을 쿼리다. SQL이 service 메서드 안에 갇혀 있어 옆 service에서 부를 길이 없다.

**한계 2 — 다른 진입점(MCP/배치/스크립트)에서 재사용 불가.** SQL이 `GoalService`의 인스턴스 메서드(`this.env` 의존) 안에 있어, HTTP 라우트 밖(예: MCP 도구, 동기화 배치, cron)에서 같은 "팀 KPI 달성률 재계산" 로직을 쓰려면 service 전체를 인스턴스화하거나 SQL을 또 복붙해야 한다. malgnai가 정확히 이 지점에서 막혀 DAO를 도입했다(HTTP API와 `server/mcp/tools.js`가 같은 `ProjectsDao`를 공유).
...
**그래서 malgnai에서 DAO로 분리했다.** 위 4가지(쿼리 산재·진입점 재사용 불가·테스트 격리·로직 매몰)가 모두 "SQL을 DAO로 내리면" 해소된다. malgnai는 `ProjectsDao`/`TasksDao`를 API·MCP가 공유하고, 멱등 upsert를 DAO 한 곳에 두며, `constructor(db)`로 테스트를 격리한다(위 "DAO 계층의 이점" 절).

**결론 — 새 Hono 프로젝트 착수 규칙.**
> **진입점이 둘 이상(HTTP + MCP/배치/cron)이거나, 같은 테이블을 여러 곳에서 만지면 — 처음부터 DAO로 시작하라.** Service에 SQL을 인라인하는 ①형은 *진입점이 HTTP 하나뿐이고 쿼리가 한 곳에서만 쓰이는 작은 앱*에만 정당하다. performance처럼 "처음엔 작아서 Service에 다 넣었는데 나중에 MCP/재사용이 필요해지는" 경로는 매우 흔하므로, 조금이라도 재사용·다진입점 냄새가 보이면 DAO를 기본값으로 둔다. 이미 Service형으로 커진 코드라면, 중복되는 쿼리(performance의 사용자 조회처럼)부터 DAO 메서드로 추출하는 점진 리팩터가 안전하다.
